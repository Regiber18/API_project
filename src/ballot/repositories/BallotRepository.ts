...
export class BallotRepository {
    ...
    public static async createBallot(ballot: Ballot): Promise<Ballot> {
        ...
        return new Promise((resolve, reject) => {
            connection.execute(query, [ballot.alumn_id, ballot.content, ballot.created_at, ballot.created_by, ballot.updated_at, ballot.updated_by, ballot.deleted], (error: any, result: ResultSetHeader) => {
                if (error) {
