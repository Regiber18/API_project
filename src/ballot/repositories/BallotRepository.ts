...
export class BallotRepository {
    ...
    public static async updateBallot(ballot_id: number, ballotData: Ballot): Promise<Ballot | null> {
        const query = 'UPDATE Ballot SET observations = ?, rating = ?,updated_at = ?, updated_by = ?, deleted = ? WHERE ballot_id = ? AND deleted = 0';
        return new Promise((resolve, reject) => {
            connection.execute(query, [ballotData.observations, ballotData.rating,ballotData.updated_at, ballotData.updated_by, ballotData.deleted, ballot_id], (error: any, result: ResultSetHeader) => {
                if (error) {
