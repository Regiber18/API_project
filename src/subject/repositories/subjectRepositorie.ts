...
export class SubjectRepository {
  ...
  public static async getSubjectRatingSpanish(): Promise<subjectRatingEspañol[]> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT r.rating_id, r.alumn_id,r.amount, r.pertenence, r.gradePertenence, sr.subject_id, s.name AS subject_name FROM Rating r JOIN SubjectRating sr ON r.rating_id = sr.rating_id JOIN Subject s ON sr.subject_id = s.subject_id WHERE r.pertenence = "Spanish" AND s.name = "Spanish"'
        , (error: any, results) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async getAllRatingId(alumn_id: number): Promise<Rating[]> {
    return new Promise((resolve, reject) => {
        const query = `
            SELECT r.amount 
            FROM Rating r 
            JOIN SubjectRating sr ON r.rating_id = sr.rating_id 
            JOIN Subject s ON sr.subject_id = s.subject_id 
            WHERE r.alumn_id = ? AND r.deleted = 0
        `;
        connection.query(query, [alumn_id],(error: any, results) => {
            if (error) {
                reject(error);
            } else {
                const ratings: Rating[] = results as Rating[];
                resolve(ratings);
            }
        });
    });
